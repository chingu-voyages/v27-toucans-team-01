...
var musicApi = require('../lib/api-music');
...
app.get('/spotifyLogin', function(req, res) {
	...
	res.redirect('https://accounts.spotify.com/authorize?' +
		querystring.stringify({
			response_type: 'code',
			client_id: musicApi.spotifyClientId,
...
app.get('/spotifyCallback', function(req, res) {
	...
	var authOptions = {
		...
		headers: {
			'Authorization': 'Basic ' + (new Buffer(musicApi.spotifyClientId + ':' + musicApi.spotifyClientSecret).toString('base64'))
		},
		json: true
	};
...
app.get('/deezerCallback', function(req, res) {
	...
	var authOptions = {
		url: 'https://connect.deezer.com/oauth/access_token.php?' +
			querystring.stringify({
				app_id: musicApi.deezerAppId,
				secret: musicApi.deezerSecretKey,
				code: code,
			}),
		json: true
	};
